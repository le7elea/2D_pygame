...
def npc_intro_level1():
    ...
    tips = [
        "Hi there, adventurer! Welcome to Level 1!",
        ...
        "Press SPACE to shoot projectiles at enemies.",
        "Collect items for Pygame tips that help with quizzes.",
    ...
    while current_tip < total_tips:
        ...
        title = title_font.render("Level 1 - Tutorial", True, LIGHT_BLUE)
        screen.blit(title, (WIDTH//2 - title.get_width()//2, 30))
        
        # Add subtitle showing this is the actual maze
        subtitle_font = pygame.font.SysFont("arial", 20, italic=True)
        subtitle = subtitle_font.render("This is the exact maze you'll be playing in Level 1!", True, YELLOW)
...
# Maze Generation, Draw Maze, Pause Menu, Quiz Screen, Save/Load Highscore, 
# Play Level, Level Complete, Game Over, and Main function
...
def play_level(level, lives):
    ...
    # --- Projectile Setup ---
    projectile_img = load_sprite("projectile.png", int(tile_size * 0.6))
    projectiles = []  # list of [pixel_x, pixel_y, dir_x, dir_y]
    last_shot_time = 0
    shot_cooldown = 400  # milliseconds
    player_direction = (1, 0)  # default facing right
    ...
    while running:
        ...
        if keys[pygame.K_UP]:
            ...
            new_pos[0] -= 1
            player_direction = (-1, 0)  # Face left
            moved = True
        elif keys[pygame.K_RIGHT]:
            new_pos[0] += 1
            player_direction = (1, 0)   # Face right
            moved = True
        ...
        if keys[pygame.K_SPACE]:
            ...
            if now - last_shot_time > shot_cooldown:
                ...
                # Only allow shooting horizontally
                if dx != 0:  # left or right only
                    proj_x = px * tile_size + tile_size // 2
                    proj_y = py * tile_size + tile_size // 2 + HUD_HEIGHT
                    projectiles.append([proj_x, proj_y, dx, 0])
                    last_shot_time = now
        ...
        if player_direction[0] == -1:  # Facing left
